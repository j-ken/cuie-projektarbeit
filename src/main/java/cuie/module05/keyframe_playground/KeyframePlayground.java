...
public class KeyframePlayground extends BorderPane {
	...
	private Circle    circleLine;
	private Circle    circleLineTwo;
	private Text      text;
	...
	private void initializeControls() {
		...
		circleLine = new Circle(250, 300, 60);
		circleLine.setStrokeWidth(4.0);
		circleLine.setStroke(Color.BLACK);
		circleLine.setFill(Color.TRANSPARENT);

		circleLineTwo = new Circle(250, 300, 60);
		circleLineTwo.setStrokeWidth(4.0);
		circleLineTwo.setStroke(Color.BLACK);
		circleLineTwo.setFill(Color.TRANSPARENT);
	...
	private void layoutControls() {
		...
		drawingPane.getChildren().addAll(circleLine, text, rectangle, line, path, pathFinder, ellipse);
	...
	private void addEventHandlers() {
		rectangle.setOnMouseClicked(event -> {
            ...
            KeyValue thirdValue = new KeyValue(rectangle.heightProperty(), 300);

            KeyFrame frame = new KeyFrame(Duration.seconds(0.5), endValue, thirdValue);
		...
		circleLine.setOnMouseClicked(event -> {
			if (timeline.getStatus().equals(Animation.Status.RUNNING)) {
				return;
			}

			DoubleProperty animatedProperty = circleLine.scaleXProperty();

			KeyValue endValue = new KeyValue(animatedProperty, 0);
